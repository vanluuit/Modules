            ...
            <div class="main">
                ...
                <div class="list_table clearfix mgt-30" id="list_table">
                    <div class="float-center">
                        {{ $nusers->appends(request()->query())->links() }}
                    </div>
                    <table class="tb table-striped">
                        <tbody>
                            ...
                            @if($nusers)
                            @foreach($nusers as $key => $nuser)
                            <tr>
                                <td class="center">
                                    <label><input type="checkbox" name="id[]" value="{{$nuser->id}}" checked="checked"></label>
                                </td>
                                <td><span class="bold_black"><a href="#">{{$nuser->id}}</a></span></td>
                                <td>{{@config('siteconfig.permission')[$nuser->confirmed]}}</td>
                                <td>{{$nuser->email}}</td>
                                <td>
                                    @if($nuser->confirmed == 1)
                                        <a href="{{route('admin.userLoginto', ['id'=>$nuser->id])}}" class="action_but" title="ログイン" href=""><i class="fa fa-lock" aria-hidden="true"></i></a>
                                    @else
                                        <a href="#" class="action_but" title="ログイン" href=""><i class="fa fa-lock" aria-hidden="true" disnable></i></a>
                                    @endif
                                    <a class="action_but" href="{{route('admin.nUserEdit', ['id'=>$nuser->id])}}"><i class="fa fa-pencil" aria-hidden="true"></i></a>
                                    <a class="action_but" onclick="return confirm('Are you sure you want to delete this item?');" href="{{route('admin.userDelete', ['id'=>$nuser->id])}}"><i class="fa fa-remove" aria-hidden="true"></i></a>
                                    <a class="action_but" href="{{route('admin.getnUserview', ['id'=>$nuser->id])}}"><i class="fa fa-search" aria-hidden="true"></i></a>
                                </td>
                            </tr>
                            @endforeach
                            @endif
